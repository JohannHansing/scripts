...
then
    ...
    echo "Usage: plot_D_over_p_vary_a_constU.sh U type a1 a2  .."
    exit 1
fi
...
do
	...
	if [ $(echo "$a == 0" | bc) -eq 1 ] 
	    then
			folder=/Users/jh/Documents/workspace-cpp/SingleParticleSimulation/sim_dataOLD/noreset/dt0.0001/t200/d0/b10
			MSD=""
			k="1.000"
	else 
        folder=/Users/jh/Documents/workspace-cpp/SPS/Release/sim_data/noreset/$type/dt0.001/t200/a$a/d0
    ...
    find $folder -name k$k -type d | while read line
		do
            file=$line/u$U/InstantValues/linear_fit_parameters$MSD.txt
			if [ -f $file ]
                then 
	...
	echo "\"tmp/plotdata_$i.txt\" u (\$1+$afrac):(\$2/6):(\$3/6) w linespoints ls 1 pt 5 lc rgb ${color[$j-1]} ti \"\$a = $afrac \\\\\\\\, b\$\", \\" >> tmp/plot.gp
    outfile+="_$a"
